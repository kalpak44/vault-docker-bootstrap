...
if [[ -f "${VAULT_INIT_FILE}" ]]; then
  ...
  vault login -no-print "${VAULT_TOKEN}" >/dev/null || true
else
...
if [[ -n "${VAULT_TOKEN:-}" ]]; then
  ...
  if [[ "${VAULT_ENABLE_KV}" == "true" ]]; then
    ...
    if ! vault secrets list -format=json | jq -e --arg m "${kv_mount}" '.[$m]' >/dev/null 2>&1; then
      ...
      vault secrets enable -path="${VAULT_KV_PATH}" kv-v2 >/dev/null
    else
  ...
  if [[ "${VAULT_WRITE_DEFAULT_POLICY}" == "true" ]]; then
    ...
    vault policy write "${VAULT_DEFAULT_POLICY_NAME}" "/opt/vault-bootstrap/policies/${VAULT_DEFAULT_POLICY_NAME}.hcl" >/dev/null
  fi
  ...
  if [[ "${VAULT_ENABLE_APPROLE}" == "true" ]]; then
    ...
    secret_id="$(vault write -field=secret_id -f auth/approle/role/${VAULT_APPROLE_NAME}/secret-id)"
